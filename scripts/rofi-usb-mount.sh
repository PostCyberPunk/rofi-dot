...
ignoredisk='mapper'
usbcheck() {
	mounteddrives="$(lsblk -rpo "name,type,size,mountpoint" | grep -v $ignoredisk | awk '$2=="part"&&$4!=""{printf "%s (%s)\t  ",$1,$3}')"
...
umountall() {
	for chosen in $(echo $(lsblk -rnpo name,size,mountpoint,fstype | grep -v "$ignoredisk" | awk 'NF==4 {print $1}')); do
		udisksctl unmount --no-user-interaction -b "$chosen"
	done
}

usbdrives="$(lsblk -rnpo name,size,mountpoint,fstype | grep -v $ignoredisk | awk 'NF==3 {print $1, $2, $3}')"
mounteddrives="$(lsblk -rnpo name,size,mountpoint,fstype | grep -v $ignoredisk | awk 'NF==4 {print $1, $2, $3, $4}')"
